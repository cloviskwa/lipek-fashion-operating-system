...
| `LIPEK_MFA_ENCRYPTION_KEY`                                                      | 32-byte key (base64 or 64-char hex) encrypting TOTP secrets at rest with AES-256-GCM                    | `R-04` (`SEC-003`) | **Required — the server refuses to boot without it** (fail closed). Generate per environment; never reuse or commit                                                                               |
| `LIPEK_MFA_ENFORCEMENT`                                                         | MFA enforcement mode: `off` (default) or `privileged`                                                   | `R-04` (`SEC-006`) | `privileged` requires accounts holding a `LIPEK_PRIVILEGED_ROLES` role to complete login with a WebAuthn factor specifically. Break-glass: set `off` + restart                                    |
| `LIPEK_PRIVILEGED_ROLES`                                                        | Comma-separated Vendure Role `code`s subject to `privileged` enforcement                                | `R-04` (`SEC-006`) | Defaults to `super-admin` (SOT §22 catalog); extend as staff roles land (`SEC-006` RBAC)                                                                                                          |
| `LIPEK_WEBAUTHN_RP_ID` / `LIPEK_WEBAUTHN_RP_NAME`                               | WebAuthn relying-party identity for passkey ceremonies                                                  | `R-04` (`SEC-002`) | Defaults `localhost` / `LIPEK` (dev only); production rpID must be the real registrable domain                                                                                                    |
| `LIPEK_WEBAUTHN_ORIGINS`                                                        | Comma-separated origins allowed to perform WebAuthn ceremonies                                          | `R-04` (`SEC-002`) | Dev default covers Admin API `:3000`, Dashboard Vite `:5173`, storefront `:3001`                                                                                                                  |
| `LIPEK_AUTH_RATE_LIMIT_MAX`                                                     | Per-minute per-IP+identifier cap on anonymous auth ceremonies (challenge/mfa-recovery endpoints)        | `R-04` (`SEC-005`) | Default `10`; layered under the general API limiter (`FOUND-019`). In-memory only until Redis-backed hardening (`OPS-007`)                                                                        |
