...
| `security-architecture.md`         | Derived       | **Created (`FOUND-019`), growing through `SEC-001`–`SEC-011`** | Operationalizes SOT §38/§52C **plus** this engagement's MFA/social-login/hardening requirements; consumes `ADR_BACKLOG.md` ADR-0006/0008 and `SIBLING_PROJECT_SECURITY_FINDINGS.md`. Currently documents only the `FOUND-019` origin-allow-list/rate-limit/headers baseline — auth/MFA/RBAC content lands in Phase 1C/1D. |
...
| `ROCKET_NEW_BUILD_BRIEF.md`            | Derived                       | **Created**           | External build brief handed to rocket.new for the platform UI layer (storefront, Dashboard extensions, PWA/mobile shells, AI surfaces). Derives from the SOT §0A–§52E and `MASTER_IMPLEMENTATION_PLAN.md`; adds no decisions of its own. Enforced against the repository by `repo-manifest.json` + `scripts/verify-manifest.mjs` |
| `repo-manifest.json`                   | Operational                   | **Created**           | Generated SHA-256 fidelity manifest of every project file, produced by `scripts/generate-manifest.mjs`. Never hand-edited                                                                                                                                                                                                        |
...
- Domain docs (`docs/domains/*`) describe _what the domain does and why_; API docs (`docs/api/*`) describe _the contract_; neither restates the other's content, they cross-link.
...
- `ADR_BACKLOG.md` owns _open_ decisions; `docs/adr/ADR-XXXX-*.md` owns _closed_ decisions. A decision exists in exactly one of the two at any time.
