...
**Progress:** Phase 0 is substantially complete (see status table below). Phase 1 has not started — `apps/server`/`apps/storefront` are not yet bootstrapped. All other tasks below remain `NOT STARTED`.
...
A pnpm monorepo: `apps/server` (Vendure Core + NestJS plugins + Dashboard), `apps/storefront` (Next.js, bootstrapped from the official Vendure starter), `apps/staff-console-extensions` (LIPEK Dashboard extensions), `apps/mobile/{customer,delivery,staff}`, `apps/ai` (Mastra), `packages/{ui,shared,schemas,graphql,config,testing}`, `docs/`, `infra/`, `scripts/`. Commerce, content, tailoring, alterations, laundry, appointments, CRM, loyalty, documents, customer-experience, analytics-events, and integrations are each a bounded-context Vendure plugin. MFA/social-login/RBAC live in a new `lipek-security` plugin. Dark/light theme tokens live once in `packages/ui` with a thin per-renderer bridge. AEO lives in the storefront's SEO layer over backend-owned content — no new service required.
...
| 1 | Monorepo, Commerce Foundation & Identity/Security Foundation | Release 1 (foundation) | `FOUND-013`–`FOUND-023`, `SEC-001`–`SEC-011`, `COM-001`–`COM-005` |
...
```text
SEC track:      SEC-001…011 (Phase 1) ──────────────────────────► SEC-012 (Phase 11)
...
|---|---|
| `CONTENT-*` (Phase 2) and `SEC-007`/`SEC-008` social login (Phase 1 tail) | Both depend only on `FOUND-013`–`FOUND-016` (monorepo + Vendure bootstrap), not on each other |
...
All 11 ADRs in the backlog have also been resolved — see `docs/implementation/ADR_BACKLOG.md` Resolution Summary and `docs/adr/ADR-0001`–`ADR-0011`. **Phase 0 is complete.** Phase 1 (`FOUND-013` onward) has not started: `apps/server` and `apps/storefront` have not been bootstrapped via `@vendure/create`, no Vendure/NestJS/Next.js dependency has been installed, and no line of application code exists yet. The monorepo skeleton (`apps/*`, `packages/*`, `infra/*`, `scripts/*` directories) and reusable assets from the prior prototype (design tokens, UI primitives, content fixtures, the Capacitor mobile scaffold) have been pre-positioned in their target locations to make `FOUND-013`–`FOUND-023` faster, but positioning a file is not the same as completing its task — each `FOUND-013`+ task remains `NOT STARTED` until its own acceptance criteria are met.
...
- **Architecture Work:** Execute the migration plan in `TARGET_REPOSITORY_STRUCTURE.md` §6 steps 1–6; establish the `lipek-security` plugin boundary.
- **Backend Work:** `@vendure/create` bootstrap, PostgreSQL wiring, migrations policy, `lipek-security` plugin (native auth, WebAuthn, TOTP, backup codes, Google/Apple external auth, RBAC scaffolding, rate limiting), event bus wiring, CI baseline.
- **Client Backend Work:** None beyond the Dashboard existing and being reachable (deep admin work starts Phase 2); staff login (including MFA enforcement for privileged accounts) must work by the end of this phase.
- **Storefront Work:** New `apps/storefront` bootstrapped from the official starter; customer login/register/OAuth redirect UX; reusable assets ported in from the prototype.
- **Database Work:** Initial PostgreSQL schema via Vendure's own migrations plus `lipek-security` tables (WebAuthn credentials, TOTP secrets — encrypted at rest, backup codes — hashed, audit log).
- **Infrastructure:** Docker Compose (Postgres, Redis stub), CI pipeline (lint/test/build), health checks.
- **Security:** This phase **is** the security foundation — see `SEC-001`–`SEC-011`.
- **Testing:** Vitest unit baseline; Vendure plugin integration test baseline; MFA enrollment/verification test suite; rate-limit test.
- **Documentation:** `local-development.md`, `migrations.md`, `security-architecture.md` (first real version), dependency register starts filling in.
- **Acceptance Criteria:** One-command local startup (`docs/implementation/local-development.md` verified); a test customer can register via email, enroll a passkey, and log in with it; a test staff account with a privileged role cannot log in without WebAuthn once enforcement mode is enabled.
- **Dependencies:** Phase 0. ADR-0006 (MFA approach) and ADR-0008 (social login pattern) must be resolved before `SEC-002`/`SEC-007`/`SEC-008` start.
- **Risks:** See Risk Register §12, items R-1, R-2, R-5.
- **Completion Gate:** No Phase 2 content-management task may start until commerce foundation + identity/security foundation both pass acceptance.
...
| `FOUND-014` | Bootstrap `apps/server` + `apps/storefront` | Run `@vendure/create` (current stable 3.x, PostgreSQL, official Next.js starter) into `apps/`, verified against current docs per `FOUND-002` | `apps/server`, `apps/storefront` | `FOUND-013` | Scaffold boots locally | Server + storefront + Dashboard all start via generated scripts |
| `FOUND-015` | Normalize pnpm workspace & Node toolchain | Align every app/package to the shared TS/ESLint/Prettier config in `packages/config` | `packages/config`, all `apps/*` | `FOUND-014` | `pnpm lint`, `pnpm typecheck` pass across workspace | Single lint/format/typecheck command covers all apps |
...
| `FOUND-022` | Define canonical event catalog & EventBus conventions | Draft `event-model.md` from SOT §40's example list; establish naming/versioning convention for domain events | `docs/architecture/event-model.md`, `apps/server` EventBus wiring | `FOUND-014` | Sample event publish/subscribe test | Convention doc exists before any plugin publishes its first real event |
...
| `SEC-002` | WebAuthn/passkey MFA (primary factor) | Implement enrollment + verification using a current WebAuthn library, per ADR-0006 decision | `apps/server/src/plugins/lipek-security`, `apps/storefront` auth UI | `SEC-001`, ADR-0006 accepted | Enrollment + login-with-passkey E2E test | A registered passkey authenticates a returning session |
| `SEC-003` | TOTP fallback + backup codes | RFC 6238 TOTP as fallback factor; single-use hashed backup/recovery codes | `apps/server/src/plugins/lipek-security` | `SEC-002` | TOTP enrollment/verify test, backup-code single-use test | TOTP works when no platform authenticator is available; backup codes are single-use and regenerable |
...
| `CONTENT-002` | Dashboard extensions for content entities | List/detail/form screens for every entity in `CONTENT-001` | `apps/staff-console-extensions` | `CONTENT-001` | Dashboard E2E test per entity | Staff can create/edit/publish each entity type |
...
| `CONTENT-004` | Homepage backend-driven composition | Section component types + ordering/visibility/scheduling per SOT §5A | `apps/server/src/plugins/lipek-content`, `apps/staff-console-extensions` | `CONTENT-003` | Section-ordering integration test | Staff can reorder/hide homepage sections without code |
...
| `ADMIN-001` | Unified Dashboard navigation shell | Ensure native Vendure pages + LIPEK extensions read as one system (SOT §20A) | `apps/staff-console-extensions` | `CONTENT-002` | Navigation smoke test | A staff member cannot tell which screens are "native" vs "custom" |
...
| `ADMIN-003` | Executive dashboard shell | Base widget layout for SOT §21.2 metrics (populated fully in Phase 7/`OPS-004`) | `apps/staff-console-extensions` | `ADMIN-001` | Widget render test | Shell exists and is extensible per metric |
| `ADMIN-004` | Staff onboarding + MFA enrollment enforcement UI | New staff accounts are guided through MFA enrollment before first privileged action | `apps/staff-console-extensions` | `SEC-006` | Onboarding-flow E2E test | No privileged staff account remains without MFA past first login |
| `ADMIN-005` | Audit log viewer | Dashboard screen surfacing `SEC-009`'s audit records, filterable by actor/action/date | `apps/staff-console-extensions` | `SEC-009` | Audit-viewer filter test | Authorized staff can review security-relevant history |
| `ADMIN-006` | Bulk import/export tooling UI | Dashboard-side upload/validation-report UI for `COM-005` | `apps/staff-console-extensions` | `COM-005` | Bulk-upload UI test | Staff can bulk-import products without developer help |
...
| `THEME-003` | Dashboard theming bridge | Apply `THEME-001` tokens to `apps/staff-console-extensions` | `apps/staff-console-extensions` | `THEME-001` | Theme-switch visual test | Dashboard extensions respect the same token set |
...
| `COM-021` | Staff order management screens | Verify payment, allocate inventory, update fulfillment, add tracking, cancel, refund, notes, timeline (SOT §21.3) | `apps/staff-console-extensions` | `COM-018` | Staff order-action E2E test | All SOT §21.3 staff actions available and permission-gated |
...
| `TAILOR-006` | Tailor assignment & workload view | Staff can assign/reassign tailors, see workload by tailor | `apps/staff-console-extensions` | `TAILOR-002` | Assignment E2E test | Workload view matches SOT §21.4 |
...
| `TAILOR-009` | Staff production dashboard | Stage checklist, due dates, overdue jobs, material requirements, QC queue (SOT §21.4) | `apps/staff-console-extensions` | `TAILOR-002` | Dashboard data-accuracy test | Overdue jobs correctly flagged |
...
| `ALTER-006` | Staff alteration queue & QC screen | Assessment queue, quality-control checklist | `apps/staff-console-extensions` | `ALTER-002` | Queue-accuracy test | Matches SOT §21 operations expectations |
...
| `LAUNDRY-004` | Pickup/delivery scheduling & assignment | Assign delivery staff to pickup/delivery tasks | `apps/server/src/plugins/laundry`, `apps/staff-console-extensions` | `LAUNDRY-003` | Assignment test | Ready for `MOBILE-003` courier app consumption |
...
| `LAUNDRY-006` | Staff laundry operations dashboard | Pickup schedule, inspection notes, cleaning method, stain notes, pressing/QC/packaging/delivery status (SOT §21.5) | `apps/staff-console-extensions` | `LAUNDRY-002` | Dashboard data-accuracy test | Matches SOT §21.5 list |
...
| `CRM-003` | Customer 360 aggregation view | Identity/Commerce/Fashion Profile/Services/Engagement/CRM/Loyalty sections (SOT §20.1) | `apps/staff-console-extensions` | `CRM-001`, `LAUNDRY-008` | Aggregation-accuracy test | Every SOT §20.1 branch populates from real data |
...
| `CRM-006` | Staff CRM backend screens | Leads, opportunities, follow-ups, quotes, notes, communication history, LTV (SOT §21.6) | `apps/staff-console-extensions` | `CRM-002` | Screen E2E test | Matches SOT §21.6 |
| `CRM-007` | Segment-driven filtering/reporting | Reporting views built on `CRM-005` | `apps/staff-console-extensions` | `CRM-005` | Report-accuracy test | Numbers match underlying data |
...
| `OPS-004` | KPI dashboard | Conversion, AOV, LTV, repeat rate, return rate, revenue by category/channel/segment (SOT §41.2) | `apps/staff-console-extensions` | `OPS-002`, `CRM-003` | KPI-calculation accuracy test | Matches SOT §41.2 list |
| `OPS-005` | Operations dashboard | Tailor workload, laundry queue, deliveries due, low stock, at-risk orders, VIP follow-ups (SOT §21.2) | `apps/staff-console-extensions` | `TAILOR-009`, `LAUNDRY-006` | Dashboard data-accuracy test | Matches SOT §21.2 list fully — this is `ADMIN-003`'s shell, now fully populated |
...
| `AI-016` | Internal copilots (CRM/Ops/Inventory/Merchandising) + management summary | "What needs my attention today?" style briefs (SOT §29), CRM/inventory/merchandising assistants (SOT §28) | `apps/ai/src/mastra/agents`, `apps/staff-console-extensions` | `OPS-004`, `OPS-005`, `CRM-007` | Summary-accuracy test | SOT §29 example brief reproducible from real data |
...
| `MOBILE-004` | Staff/admin tablet validation | Confirm Vendure Dashboard usability on tablet form factors for on-the-go admin use, per ADR-0007 outcome | `apps/staff-console-extensions` | ADR-0007 | Tablet usability test | No blocking usability gap for staff mobile use |
...
Security is not a Phase 11 afterthought — the foundation (`SEC-001`–`SEC-011`) lands in **Phase 1**, before any content or commerce feature work, because customer/staff identity underlies everything else. Sequence: native email+password auth → WebAuthn/passkey MFA (primary, "most advanced practice") → TOTP fallback + backup codes → email recovery channel (pending ADR-0002) → auth-endpoint rate limiting → RBAC with mandatory WebAuthn for privileged staff roles → Google OAuth → Apple OAuth → audit logging → secrets-rotation policy → dependency/vulnerability scanning in CI. `SEC-012` in Phase 11 is the production security review and penetration-test gate, not the first time security is considered. Every decision here traces to `ADR-0006`/`ADR-0008` and the concrete findings in `SIBLING_PROJECT_SECURITY_FINDINGS.md` (no sibling project had working MFA/social login to copy — this is confirmed as original build work, not integration work).
