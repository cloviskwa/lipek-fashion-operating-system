...
| `R-03` | **Specify service stage vocabularies + timeline serialization (ADR)** | — | — | 1–2 d | **DONE** (7 Sep) — [`ADR-0014`](../adr/ADR-0014-service-workflow-stage-vocabularies.md) **Accepted**; all six questions decided as proposed; reference engine + tests in `src/common/service-workflow` |
...
| `R-06` | `TailoringPlugin` — jobs, configurations, measurements, production timeline, fittings | 6 | 73 | 8–12 d | **DONE** (7 Sep) — server side, live-E2E-verified; Dashboard transition buttons + storefront tracking UI are Phase 2 polish |
...
| Date | Change |
| --- | --- |
| 2026-09-07 | `R-06` completed (server side) — `TailoringPlugin`. Six entities transcribed from the surviving schema with its constraint names pinned (UNIQUE one-to-one `REL_` indexes on configuration/timeline/measurement-profile, the `REL_`+`FK_` pair per relation, `IDX_` names elsewhere), so the boot check passes with zero schema-mismatch warnings and no migration. Production stages run through the shared `stage-machine` engine (`ADR-0014`): vocabulary enforced on write, forward skips recorded by absence in `stageTimestamps`, the single permitted backward loop (`FINAL_FITTING → ADJUSTMENTS`) honoured, operational stages restricted to the assigned tailor or a supervisor, cancellation orthogonal via the job's `cancelledAt`. Notification records (`QUEUED`, `provider-pending`) are written for handover stages. Fittings reserve real capacity through the AppointmentsPlugin's BookingService (`R-05`), linking via the `tailoring_job` subject pair — the two plugins' intended seam, exercised end to end. Events: `TailoringJobCreated`, `FittingScheduled`, and one `TailoringStageTransitionEvent` carrying `fromStage`/`toStage` (the per-transition catalog entries derive from it). Shop API: own jobs' progress, own measurement profiles (customer-provided source). Admin API: job/assignment/cancellation, stage transitions, configuration upsert, measurements, fittings, quality-control verdict, behind a new `Tailoring` CRUD permission. Dashboard: job roster and fittings calendar. Verified: typecheck clean, full suite 40 tests, live-server E2E (`tests/tailoring/tailoring-live.e2e.mjs`, 7 checks), dashboard Vite build, clean boot. |
